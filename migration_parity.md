...
| ESM handler loading (`.mjs`, TLA) | shim:`src/runtime/user-function.ts` | L2 + L3 | golden esm variants; `ERR_REQUIRE_ESM` guard formerly in `integration_tests/serverless.yml`; real AWS Lambda resource coverage relies on the end-to-end test suites → port | pending | datadog-lambda-js |
