import { deterministicMd5Hash, hexToBinary } from "./step-functions-trace-context"
...
describe("test_deterministicMd5Hash", () => {

  it("test same hashing is generated as logs-backend for a random string", () => {
    const actual = deterministicMd5Hash("some_testing_random_string")
  ...
  it("test same hashing is generated as logs-backend for an execution id", () => {
    const actual = deterministicMd5Hash("arn:aws:states:sa-east-1:601427271234:express:DatadogStateMachine:acaf1a67-336a-e854-1599-2a627eb2dd8a:c8baf081-31f1-464d-971f-70cb17d041f4")
  ...
  it("test same hashing is generated as logs-backend for another execution id", () => {
    const actual = deterministicMd5Hash("arn:aws:states:sa-east-1:601427271234:express:DatadogStateMachine:acaf1a67-336a-e854-1599-2a627eb2dd8a:c8baf081-31f1-464d-971f-70cb17d01111")
  ...
  it("test same hashing is generated as logs-backend for execution id # state name # entered time", () => {
    const actual = deterministicMd5Hash("arn:aws:states:sa-east-1:601427271234:express:DatadogStateMachine:acaf1a67-336a-e854-1599-2a627eb2dd8a:c8baf081-31f1-464d-971f-70cb17d01111#step-one#2022-12-08T21:08:19.224Z")
  ...
  it("test hashing different strings would generate different hashes", () => {
    ...
    for (let i = 0; i < times; i++) {
      for (let j = i + 1; j < times; j++) {
        expect(deterministicMd5Hash(i.toString())).not.toMatch(deterministicMd5Hash(j.toString()));
      }
    }
  });
  ...
  it("test always leading with 0", () => {
    for (let i = 0; i < 20; i++) {
      expect(deterministicMd5Hash(i.toString()).substring(0, 1)).toMatch("0");
    }
  });
})
