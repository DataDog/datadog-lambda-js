...
describe("EventBridgeSQSEventTraceExtractor", () => {
  describe("extract", () => {
    ...
    it("extracts trace context with valid payload", () => {
      ...
      const extractor = new EventBridgeSQSEventTraceExtractor(tracerWrapper, {} as TraceConfig);
    ...
    it("extracts trace context from Step Function EventBridge-SQS event", () => {
      ...
      expect(traceContext?.[0].toTraceId()).toBe("7858567057595668526");
      expect(traceContext?.[0].toSpanId()).toBe("3674709292670593712");
      expect(traceContext?.[0].sampleMode()).toBe("1");
      expect(traceContext?.[0].source).toBe("event");
    });
  });
