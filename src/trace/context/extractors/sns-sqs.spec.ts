...
describe("SNSSQSEventTraceExtractor", () => {
  describe("extract", () => {
    ...
    it("extracts trace context with valid payload with String Value", () => {
      ...
      const extractor = new SNSSQSEventTraceExtractor(tracerWrapper, {} as TraceConfig);
    ...
    it("extracts trace context from AWSTraceHeader with valid payload", () => {
      ...
      expect(traceContext?.[0].toTraceId()).toBe("5027664352514971920");
      expect(traceContext?.[0].toSpanId()).toBe("1797917631284315439");
      expect(traceContext?.[0].sampleMode()).toBe("1");
      expect(traceContext?.[0].source).toBe("event");
    });
    ...
    it("extracts trace context from Step Function SNS-SQS event", () => {
      ...
      expect(traceContext?.[0].toTraceId()).toBe("1657966791618574655");
      expect(traceContext?.[0].toSpanId()).toBe("5100002956473485303");
      expect(traceContext?.[0].sampleMode()).toBe("1");
      expect(traceContext?.[0].source).toBe("event");
    });
  });
