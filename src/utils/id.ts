...
// HACK - copied from dd-trace as it's not exported
...
function createBuffer(value: string): Uint8Array {
...
function fromString(str: string, raddix: number): Uint8Array {
...
function pseudoRandom(): Uint8Array {
  if (batch === 0) {
    randomFillSync(data);
  }
...
export function id(value: string, radix?: number) {
  new Identifier(value, radix);
}
